...
- Canvas uses fixed logical dimensions (1100x620) with uniform scaling
- DrawUIBox accepts text arrays: `DrawUIBox(["line1", "line2"], centerX, centerY)`
...
### HUD Rendering
- Score uses `Constants.UI.HUD_SCORE_SIZE` for text size
- Ship icons use static Vector arrays (not live ship points)
- Ship icons use `Constants.UI.HUD_SHIP_ICON_SIZE` for scale
- Ship spacing uses `Constants.UI.HUD_SHIP_SPACING`
- Mobile: HUD tap area (top 60px) triggers pause toggle
